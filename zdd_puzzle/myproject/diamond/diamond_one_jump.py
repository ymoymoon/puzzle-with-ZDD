...
def next_all(F: ZDD) -> ZDD:
    ...
    for fr, ov_to_list in JUMPS_BY_FR.items():
        ...
        af = ZDD.empty
        for ov,to in ov_to_list:
            # af = af + F_fr.onset(ov).offset(to).change(fr).change(ov).change(to) <- 間違っているのでは
            af = af + F_fr.onset(ov).offset(to).change(fr).change(to)
...
# def trace_solution(history: list[ZDD], min_moves: int) -> list[dict]: <- うまく動作しない，前向きではダメか
#     """
#     history を使って前向きに手順を1つ復元する．
#     history[t] = ちょうど t 手後の新規到達盤面族

#     1ターンには「1歩移動」または「連続ジャンプ（1回以上）」のどちらかがあり，
#     連続ジャンプの場合は経路上の各ジャンプを個別のアニメーションフレームに分解する．
#     """
#     if min_moves <= 0:
#         return []
...
#         # (A) 1歩移動を試す
#         for fr, to in SINGLE_STEPS:
#             if fr not in board or to in board:
#                 continue
#             next_board = (board - {fr}) | {to}
#             chk = next_hist
#             for pid in sorted(next_board): chk = chk.onset(pid)
#             for pid in POS2ID.values():
#                 if pid not in next_board: chk = chk.offset(pid)
#             if int(chk.exact_count) > 0:
#                 turn_no += 1
#                 steps.append({
#                     "turn": turn_no, "step": t+1,
#                     "fr_user": ID2POS[fr], "ov_user": None, "to_user": ID2POS[to],
#                     "fr_pid": fr, "ov_pid": None, "to_pid": to,
#                     "board": frozenset(next_board), "type": "step",
#                 })
...
#         # (B) 連続ジャンプを試す: 各起点ペグについて経路探索
#         active_pegs = [p for p in board]
#         for fr in active_pegs:
#             # まず候補となる「最終盤面」を next_hist から1つ取り出して確認するのではなく，
#             # fr から到達しうる全終端盤面を求め，next_hist に含まれるものを探す
#             path = _find_reachable_match(board, fr, next_hist)
#             if path is not None and len(path) > 0:
#                 # path = [(ov1,to1), (ov2,to2), ...] をフレームごとに展開
#                 cur_board = board
#                 cur_pos   = fr
#                 for ov, to in path:
#                     nb = (cur_board - {cur_pos, ov}) | {to}
#                     turn_no += 1
#                     steps.append({
#                         "turn": turn_no, "step": t+1,
#                         "fr_user": ID2POS[cur_pos], "ov_user": ID2POS[ov], "to_user": ID2POS[to],
#                         "fr_pid": cur_pos, "ov_pid": ov, "to_pid": to,
#                         "board": frozenset(nb), "type": "jump",
#                     })
#                     cur_board, cur_pos = nb, to
#                 board = cur_board
#                 found = True
#                 break
...
def trace_solution(history: list[ZDD], min_moves: int) -> list[dict]:
    ...
    board = frozenset(GOAL_PEGS)
    steps = []
    turn_no = 0

    for t in range(min_moves):
        prev_hist = history[min_moves - 1 - t]
        found = False

        # (A) 1歩移動を試す
        for fr, to in SINGLE_STEPS:
            if fr in board or to not in board:
                continue
            prev_board = (board - {to}) | {fr}
            chk = prev_hist
            for pid in sorted(prev_board): chk = chk.onset(pid)
            for pid in POS2ID.values():
                if pid not in prev_board: chk = chk.offset(pid)
            if int(chk.exact_count) > 0:
                turn_no += 1
                steps.append({
                    "turn": min_moves + 1 - turn_no, "step": min_moves - t + 1,
                    "fr_user": ID2POS[fr], "ov_user": None, "to_user": ID2POS[to],
                    "fr_pid": fr, "ov_pid": None, "to_pid": to,
                    "board": frozenset(board), "type": "step",
                })
        ...
        # 1回だけに制限した ver
        for fr, ov, to in SINGLE_JUMPS:
            if fr in board or ov not in board or to not in board:
                continue
            prev_board = (board - {to}) | {fr}
            chk = prev_hist
            for pid in sorted(prev_board): chk = chk.onset(pid)
            ...
            if int(chk.exact_count) > 0:
                turn_no += 1
                steps.append({
                    "turn": min_moves - turn_no, "step": min_moves - t,
                    "fr_user": ID2POS[fr], "ov_user": ov, "to_user": ID2POS[to],
                    "fr_pid": fr, "ov_pid": ov, "to_pid": to,
                    "board": frozenset(board), "type": "step",
                })
                board = frozenset(prev_board)
                found = True
...
# def _find_reachable_match(board: frozenset, fr: int, target_zdd: ZDD):
#     """
#     fr から連続ジャンプして到達できる全終端盤面を幅優先で列挙し，
#     target_zdd に含まれる最初の盤面への経路を返す．
#     戻り値: [(ov,to), ...] または None
#     """
...
#     while queue:
#         cur_board, cur_pos, path = queue.popleft()

#         # 現在の盤面が target_zdd に含まれるか確認（path が空でも候補）
#         if path:
#             chk = target_zdd
#             for pid in sorted(cur_board): chk = chk.onset(pid)
#             for pid in POS2ID.values():
#                 if pid not in cur_board: chk = chk.offset(pid)
#             if int(chk.exact_count) > 0:
#                 return path

#         for ov, to in JUMPS_BY_FR.get(cur_pos, []):
#             if ov not in cur_board or to in cur_board:
#                 continue
#             nb = (cur_board - {cur_pos, ov}) | {to}
#             if nb in visited:
#                 continue
#             visited.add(nb)
#             queue.append((nb, to, path + [(ov, to)]))
...
def build_anim_json(move_steps: list[dict], px: int = 36) -> str:
    ...
    frames = [{"pegs": sorted(INIT_PEGS), "fr": None, "ov": None, "to": None,
               "label": "初期盤面", "desc": ""}]
    # 同一ターン内のジャンプ数を数えてサブステップ表示に使う
    turn_counts: dict[int,int] = _dd(int)
    for s in move_steps:
        turn_counts[s["step"]] += 1
    turn_seen: dict[int,int] = _dd(int)
    ...
    for s in move_steps:
        ...
        desc += str(s['to_user'])
        turn_seen[s["step"]] += 1
        total_in_turn = turn_counts[s["step"]]
        label = "手 "+str(s["step"])+" ("+s["type"]+")"
        if total_in_turn > 1:
            label += " ["+str(turn_seen[s["step"]])+"/"+str(total_in_turn)+"]"
        frames.append({
            "pegs": sorted(s["board"]),
            "fr":   s["fr_pid"],
            "ov":   s["ov_pid"],
...
def generate_html(min_moves, log, wall_total, cpu_total,
    ...
    step_tbl = ('<div class="table-wrap"><table>'
                '<caption>解の手順（座標 (q,r)） · 手=ターン番号，連続ジャンプは同じ手番号内で複数行</caption>'
                '<thead><tr><th>#</th><th>手</th><th>移動元</th><th>飛越え</th>'

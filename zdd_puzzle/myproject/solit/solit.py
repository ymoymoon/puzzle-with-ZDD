...
# ── 使用する盤面を選択 ──
VALID, INIT_HOLE, GOAL_POS, GRID_SIZE = make_english_board()
...
def build_reachable():
    ...
    return reachable, log, time.perf_counter() - t0, time.process_time() - c0
...
def solve_with_reachable(reachable: list[ZDD]):
    ...
    return F, log, time.perf_counter() - t0, time.process_time() - c0
...
def log_table(log: list[dict], caption: str) -> str:
    rows = "".join(
        f"<tr><td>{e['step']}</td>"
        f"<td>{e['wall']:.2f}</td><td>{e['cpu']:.2f}</td>"
...
<div class="table-wrap">
  <table>
    ...
    <thead><tr>
      <th>ステップ</th><th>実時間 (s)</th><th>CPU時間 (s)</th>
      <th>盤面数</th><th>ZDDノード数</th>
    </tr></thead>
    <tbody>{rows}</tbody>
  </table>
</div>"""
...
def generate_html(bwd_log, bwd_time, bwd_cpu, 
                  reachable_inits, num_reachable, move_steps,
                  out_path="cputime.html"):
    from pathlib import Path
...
<div class="summary">
  <div class="kpi"><div class="label">逆向き 実時間</div>
    <div class="value">{bwd_time:.1f}<span class="unit">s</span></div></div>
  <div class="kpi"><div class="label">逆向き CPU時間</div>
    <div class="value">{bwd_cpu:.1f}<span class="unit">s</span></div></div>
...
<section>
  ...
  {log_table(bwd_log, f"ゴールから逆向きに展開 · 実時間 {bwd_time:.2f}s · CPU {bwd_cpu:.2f}s")}
</section>
...
if __name__ == "__main__":
    print("逆向き探索...")
    reachable, bwd_log, bwd_time, bwd_cpu = build_reachable()
    print(f"  完了  実時間={bwd_time:.2f}s  CPU={bwd_cpu:.2f}s")
    ...
    print("HTML 生成中...")
    generate_html(
        bwd_log=bwd_log, bwd_time=bwd_time, bwd_cpu=bwd_cpu,

...
# to ごとに手をグループ化: 逆向き探索で onset(to) を1回だけ呼んで共有するため
...
def prev_all(F: ZDD) -> ZDD:
    """
    盤面族 F に対して全合法手を逆向きに適用した結果の族（和集合）を返す．

    手 (fr, ov, to) の逆操作:
      「to にペグがあり fr と ov が空き」の盤面から
      to を除いて fr と ov にペグを戻す

    ZDD演算（forward の onset/offset/change の対称):
      onset(to)   : to にペグがある盤面のみ選択
      offset(fr)  : fr が空きの盤面のみ選択
      offset(ov)  : ov が空きの盤面のみ選択
      change(to)  : to のペグを除去
      change(fr)  : fr にペグを追加
      change(ov)  : ov にペグを追加

    最適化: MOVES_BY_TO で to ごとに onset(to) を1回だけ共有
            support_vars() で to が F に存在しない手をスキップ
    """
...
def solve_backward() -> list[ZDD]:
    """
    ゴール盤面から逆向きに31ステップ展開し，各ステップの盤面族ZDDをリストで返す．
    history_bwd[0] = ゴール盤面族，history_bwd[t] = t手前の盤面族．
    """
    t0 = time.perf_counter()
    ...
    # ゴール盤面族: CENTER のみペグの状態1つだけ
    F = ZDD.from_sets([sorted(GOAL_STATE)])
    history_bwd: list[ZDD] = [F]
    print(f"  ゴール盤面数: {int(F.exact_count)}")

    for step in range(N_STEPS):
        F = prev_all(F)
        cnt = int(F.exact_count)
        print(f"  逆ステップ {step+1:2d} ({elapsed()})  到達盤面数: {cnt}")
        history_bwd.append(F)
        if cnt == 0:
            print("  到達可能な盤面がなくなりました")
            break
...
def enumerate_reachable_inits(history_bwd: list[ZDD]) -> ZDD:
    """
    逆向き探索の結果からゴールに到達可能な初期盤面を列挙する．

    ゴールに到達可能な初期盤面
      = history_bwd[N_STEPS]（31手逆向きに戻った盤面族）
        ∩ 「ペグ数が32個の盤面」
...
if __name__ == "__main__":
    ...
    # ── 逆向き探索（到達可能な初期盤面を列挙） ───────────────
    print("\n" + "=" * 55)
    print("【逆向き探索】ゴールから逆向きに展開")
    print("-" * 55)
    history_bwd = solve_backward()

    reachable = enumerate_reachable_inits(history_bwd)
    num_init = int(reachable.exact_count)
    print(f"\n  ゴールに到達可能な初期盤面数（ペグ32個）: {num_init}")

    if num_init > 0 and num_init <= 20:
        print("\n  到達可能な初期盤面一覧:")
        for i, board_vars in enumerate(reachable.enumerate()):
            show_board(frozenset(board_vars), label=f"初期盤面 #{i+1}")
            print()
    elif num_init > 20:
        print(f"  (多数のため最初の5件のみ表示)")
        for i in range(5):
            board_vars = reachable.unrank(i)
            show_board(frozenset(board_vars), label=f"初期盤面 #{i+1}")
            print()